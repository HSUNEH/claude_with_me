...
# 또는 직접 /init-project 실행
```

### 초기 세팅 흐름 (/init-project)

처음 실행하면 시스템이 자동으로 감지하고 `/init-project` 실행을 안내합니다.
...
→ /init-project 를 실행하여 프로젝트 정보를 입력하세요
    │
    ▼
사용자: /init-project
    │
    ▼
[Q1] 프로젝트 개요 (이름, 설명, 신규/기존)
[Q2] 기술 스택 (언어, 프레임워크, DB, ORM)
[Q3] 프로젝트 구조 (디렉토리, 모노레포 여부)
[Q4] 코딩 규칙 (린터, 네이밍, 컨벤션)
[Q5] 에러 처리 & 보안 (패턴, 인증 방식)
[Q6] 테스트 (프레임워크, 디렉토리, 커버리지)
    │
    ▼ (기존 프로젝트면 코드 자동 분석 후 확인)
    │
    ├── config.yml 자동 생성 (프로젝트 맞춤)
    ├── 매뉴얼 챕터 6개 자동 생성
    └── .initialized 마커 생성
    │
    ▼
✅ 세팅 완료 → 이제 정상 개발 워크플로우 작동
```

기존 프로젝트에 적용하는 경우, 코드를 자동 분석하여 기술 스택, 디렉토리 구조, 린터 설정 등을 감지한 뒤 확인만 받으므로 더 빠르게 진행됩니다.
