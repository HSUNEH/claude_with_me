...
≤ 20%   > 20%
    ...
    └─────┤
          ▼
4. 산출물 생성 → .cwm/docs/briefs/{YYMMDD}-{주제}.md
...
- 현재까지 내용을 `.cwm/docs/briefs/{YYMMDD}-{주제}-incomplete.md`로 저장
- 호출자에게 "cancelled" 반환
...
DATE=$(date +%y%m%d)
# 주제는 kebab-case
mkdir -p "$PROJECT_ROOT/.cwm/docs/briefs"
# 브리프를 "$PROJECT_ROOT/.cwm/docs/briefs/${DATE}-${TOPIC}.md" 로 Write
```
...
```
📄 브리프 저장: .cwm/docs/briefs/{YYMMDD}-{주제}.md
