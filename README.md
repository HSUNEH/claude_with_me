...
**설치 후 최초 1회 실행**하면 CWM 훅이 동작합니다.

### 2. `/cwm:planwithme` 또는 간단한 수정

| 상황 | 방법 | 예시 |
|------|------|------|
| **큰 작업** | `/cwm:planwithme {작업명}` | `/cwm:planwithme 로그인-기능` → PLAN·CONTEXT·CHECKLIST 생성 → 승인 |
| **간단한 수정** | 그냥 요청 | `이 변수 이름 바꿔줘` → 바로 진행 |
| **명시적 우회** | `간단:` 접두어 | `간단: 로고 이미지 교체해줘` → 플랜 없이 즉시 진행 |

### 3. 진행

- **플랜 있는 작업**: 승인 → `/clear` → 구현 시작 → 체크리스트 따라 진행
- **간단한 작업**: 바로 수정 완료 (1-2파일 수정은 플랜 불필요)
