...
프로젝트 디렉토리에서 터미널을 열고 아래 두 단계를 실행하세요:

```bash
# 1. 설치
git clone https://github.com/HSUNEH/dev_sys_template && bash dev_sys_template/install.sh

# 2. Claude Code 시작 후 /setup 실행
claude
```

Claude Code가 시작되면 `/setup`을 입력하여 초기화 위저드를 실행하세요.

<details>
<summary>Claude Code에서 직접 설치 (대체 방법)</summary>

아래 명령어를 Claude Code에 그대로 붙여넣으세요:

```
https://github.com/HSUNEH/dev_sys_template 을 git clone 해서 서브폴더로 다운받고, 그 안의 .claude/ 폴더와 docs/ 폴더를 현재 디렉토리에 복사한 뒤 실행 권한을 설정해줘.
```

실행 후 `/exit`으로 Claude Code를 종료하고 다시 시작한 뒤, `/setup`을 입력하세요.
</details>
...
5단계 대화형 위저드. Phase별로 질문 → 사용자 응답 → 확인을 반복하며 진행합니다.

<img alt="/setup 위저드 데모" width="600" src="./demos/assets/setup.gif" />

### 2. 계획 모드 — 체계적 개발

코드 변경 요청 시 자동으로 계획 수립을 유도하고, 승인 후 컨텍스트를 정리하여 구현을 시작합니다.
...
<p align="center">
  <img alt="워크플로우 다이어그램" width="700" src="./demos/assets/workflow.png" />
</p>
...
# 또는 수동으로:
cp -r dev_sys_template/.claude  your-project/.claude/
cp -r dev_sys_template/docs     your-project/docs/
cp dev_sys_template/CLAUDE.md   your-project/CLAUDE.md
chmod +x your-project/.claude/hooks/*.sh your-project/.claude/hooks/lib/*.sh
```

설치 후 Claude Code를 종료(`/exit`)하고 다시 시작한 뒤 `/setup`을 실행하세요.

> **주의:** 재시작해야 `/setup` 등 스킬이 인식됩니다.
